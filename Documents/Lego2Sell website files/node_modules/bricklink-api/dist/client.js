...
class Client {
  ...
  dispatch(req) {
    ...
    const oauthHelper = new _oAuthHelper.OAuthHelper(this.consumer_key, this.token);
    oauthHelper.sign(resourceURL, req, this.consumer_secret, this.token_secret);
    init.headers['authorization'] = oauthHelper.header;
    const promise = (0, _nodeFetch.default)(resourceURL, init).then(response => response.json()).then(
    /**
     * @param {any} payload Any object
     */
    payload => {
      if (payload.meta.code >= 300) {
        const error = new _brickLinkApiError.BrickLinkApiError(payload.meta);
  ...
  getCatalogItem(itemType, itemNumber) {
    const req = _catalogItem.CatalogItem.get(itemType, itemNumber);
  ...
  getPriceGuide(itemType, itemNumber, params) {
    const req = _priceGuide.PriceGuide.get(itemType, itemNumber, params);
  ...
  getKnownColors(itemType, itemNumber) {
    const req = _knownColor.KnownColor.get(itemType, itemNumber);
  ...
  getItemImage(itemType, itemNumber, colorId) {
    const req = _itemImage.ItemImage.get(itemType, itemNumber, colorId);
  ...
  getItemSubset(itemType, itemNumber, params) {
    const req = _subsets.Subset.get(itemType, itemNumber, params);
  ...
  getItemSuperset(itemType, itemNumber, params) {
    const req = _supersets.Superset.get(itemType, itemNumber, params);
